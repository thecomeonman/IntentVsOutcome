...
Using pitch control, we can estimate the probability of players controlling the ball in various parts of the pitch so we could simulate passes that didn't actually happen to estimate how much value a player _could_ have added from passes they _could_ have received. This is not the same as estimating the value of their off ball actions but more like an adjustment for the fact that not every player in the team will receive as many passes, good opportunities to pass to them might get missed because of reasons beyond their control, etc.

This is a thinking out loud kind of post. A bunch of charts and numbers are here, mostly self-explanatory so I haven't talked about some of them too much. The point is to get the idea across. I probably want to keep thinking about this some more before doing a version with nice looking vizes, etc.
...
Pitch control tells you the probability of a team and its players controlling the ball at various parts of the pitch if the ball were to be passed there. This is done for each instant of the game and changes based on the psitions of the players and their movement.
...
We can rotate the pitch 180 degrees and get the PV values for the respective part of the pitch for the opposition for them attacking right to left.
...
I chose this example because it also highlights the problem I mentioned above while hopefully also giving you an idea of the concept.
...
- From the earlier histogram, we saw most of the pitch is usually negative expected delta PV so it isn't surprising that this distribution is completely in the negative. What this means is that a random pass is more likely to help the defending team than an attacking team.

Let us instead focus on the maximum expected delta PV gain at any instant as that is indicative of the best option to pass for the attacking team.
...
pMaxPV = ggplot() + 
  geom_col(
     data = copy(dtMaxPV)[, Facet := 'Actual'],
...
print(
  kable(
    dtMaxPV[TeamMaxDeltaPV >= -0.01 & TeamMaxDeltaPV <= 0.02, list(HighTeamMaxDeltaPVActual_pct = paste0(100 * round(sum(N_pct),2), '%')), Team],
    format = 'html'
  )
)
...
This means most of the time the optimal action available to the attacking team has very low values of `ExpectedAttackingDeltaPV_xy`, if at all. The proportion of time between -1% and 2% is -

```{r PositiveEPVGT1, echo=FALSE}

(
  kable(
    dtMaxPV[TeamMaxDeltaPV >= -0.01 & TeamMaxDeltaPV <= 0.02, list(LowTeamMaxDeltaPVActual_pct = paste0(100 * round(sum(N_pct),2), '%')), Team],
    format = 'html'
  )
)
...
- Both team have a preference towards the left side of the pitch in the final third.
   ...
   dtPositiveProportion2[, 
      list(
         TeamPositiveProportionActual = round(seq(
            dtPositiveProportion2[, min(TeamPositiveProportionActual)],
            dtPositiveProportion2[, max(TeamPositiveProportionActual)],
            0.01
         ),2)
      ),
      list(AttackingTeam, Team)
...
setnames(
  dtPositiveProportion2,'TeamPositiveProportionActual','TeamPositiveProportion'
)
...
# dtPositiveProportion2[, Type := 'Actual']
...
pPositiveProportion = ggplot() + 
   ...
   geom_col(
      data = copy(dtPositiveProportion)[, Facet := 'Actual - Ideal'],
      aes(x = TeamPositiveProportion, y = N_pct.actual - N_pct.ideal, fill = Team, group = Team), 
      position = 'dodge'
   ) + 
   geom_col(
      data = copy(dtPositiveProportion)[, Facet := 'Actual'],
   ...
   dtMaxPV2[, 
      list(
         TeamMaxDeltaPVActual = round(seq(
            dtMaxPV2[, min(TeamMaxDeltaPVActual)],
            dtMaxPV2[, max(TeamMaxDeltaPVActual)],
            0.01
         ),2)
      ),
      list(AttackingTeam, Team)
...
setnames(
  dtMaxPV2, 'TeamMaxDeltaPVActual','TeamMaxDeltaPV'
)
dtMaxPV = merge(
  dtMaxPV,
  dtMaxPV2,
...
pMaxPV = ggplot() + 
  ...
  geom_col(
     data = copy(dtMaxPV)[, Facet := 'Actual - Ideal'],
...
dtPitchTrend2 = dtPitchTrend2[,
  ...
  list(
    AttackingTeam, 
    TeamMaxDeltaPVX = TeamMaxDeltaPVActualX, 
    TeamMaxDeltaPVY = TeamMaxDeltaPVActualY
  )
]
...
dtPitchTrend[, ActualMinusIdeal := as.character(sign(N_pct.actual - N_pct.ideal))]
dtPitchTrend[ActualMinusIdeal == '-1', ActualMinusIdeal := '-']
dtPitchTrend[ActualMinusIdeal == '1', ActualMinusIdeal := '+']
dtPitchTrend[ActualMinusIdeal == '0', ActualMinusIdeal := '']
...
pPitchTrend = ggplot() +
    ...
    geom_jitter(
        data = dtPitchTrend[N_pct.actual != 0][, list(V1 = 1:N.actual, Facet = 'Actual'), list(TeamMaxDeltaPVX, TeamMaxDeltaPVY, AttackingTeam)],
        aes(
            x = TeamMaxDeltaPVX, y = TeamMaxDeltaPVY,
            # size = N_pct
        ), 
...
dtOriginTarget = dtPVDelta[(TeamMaxDeltaPVActual) > TeamDeltaPVThreshold][Team == AttackingTeam][order(Frame)][
  # c(T,diff(Frame) > 100)
][,
  .SD[1],
  Frame
...
kable(
  dtPlayerLevelMetrics[, 
    list(
      HighPlayerMaxDeltaPVActual_pct = sum(N_pct[PlayerMaxDeltaPVActual >= PlayerDeltaPVThreshold])
    ),
    list(Player = TeamLabel)
  ][
    order(-HighPlayerMaxDeltaPVActual_pct),
    list(
      HighPlayerMaxDeltaPVActual_pct = paste0(100 * round(HighPlayerMaxDeltaPVActual_pct,2),'%'),
      Player
    )
  ], format = 'html'
)
...
ggplot(dtPlayerLevelXY) + 
   geom_pitch(cLineColour = '#CCCCCC', cPitchColour = 'transparent') +
   geom_jitter(aes(x= PlayerMaxDeltaPVActualX, y = PlayerMaxDeltaPVActualY), size = 0.2, width = 0.5, height = 0.5) +
...
kable(
  dtPlayerLevelMetrics[, 
    list(
      HighTeamMaxDeltaPVActual_pct = sum(N_pct[TeamMaxDeltaPVActual >= TeamDeltaPVThreshold])
    ),
    list(Player = TeamLabel)
  ][
    order(-HighTeamMaxDeltaPVActual_pct),
    list(
      HighTeamMaxDeltaPVActual_pct = paste0(100 * round(HighTeamMaxDeltaPVActual_pct,2),'%'),
      Player
    )
  ], format = 'html'
)
...
dtPlayerLevelXY = merge(
   ...
   dtPVDelta[Team == AttackingTeam][,.SD[1],list(Frame,Tag)][TeamMaxDeltaPVActual > TeamDeltaPVThreshold], 
...
ggplot(dtPlayerLevelXY) + 
   geom_pitch(cLineColour = '#CCCCCC', cPitchColour = 'transparent') +
   geom_jitter(aes(x= TeamMaxDeltaPVActualX, y = TeamMaxDeltaPVActualY), width = 0.5, height = 0.5) +
   facet_wrap(~TeamLabel, ncol = 4) + 
   coord_fixed() + 
   scale_color_continuous(low = 'black', high = 'red') +
   theme_pitch()
...
# dtPVDelta[AttackingTeam == Team, .SD[1], list(Frame)][, .SD[which.max(TeamMaxDeltaPVActual)], PossessionID]
